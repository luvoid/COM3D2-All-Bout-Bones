...
**All _children_ (with a `_yure_` / `_yure_` *descendant*) of the first yure bone's *parent* will become dynamic bones.** If there are multiple yure branches, you can make sure they all become dynamic by adding a "dummy" yure bone (that doesn't actually have `_yure_` in its name) to their first common ancestor. 
